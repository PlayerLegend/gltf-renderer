...
int main (int argc, char * argv[])
{
    ...
    if (!glb_toc_load_from_interface(&toc, &fd_read.interface))
    {
    ...
    range_const_char json_text =
	{
	    .begin = (const char*) toc.json->data,
	    .end = (const char*) toc.json->data + toc.json->length,
	};

    json_value * json_root = json_parse(json_text.begin, json_text.end);
    
    if (!gltf_from_json (&gltf, json_root))
    {
	log_fatal ("Failed to parse json");
    }
	...
	for_range(mesh_primitive, mesh->primitives)
	{
	    ...
	    assert (!position_accessor->buffer_view->buffer->data);

	    range_unsigned_char file_bin =
	    {
		.begin = toc.bin->data,
		.end = file_bin.begin + toc.bin->length,
	    };
	    
	    range_unsigned_char buffer_bytes =
	    {
		.begin = file_bin.begin,
		.end = buffer_bytes.begin + position_accessor->buffer_view->buffer->byte_length,
	    };
	    
	    range_unsigned_char buffer_view_bytes =
	    {
	    ...
	    range_unsigned_char accessor_bytes =
	    {
